...
import { observer } from '@ecomplus/storefront-template/template/js/lib/lazy-load'
...
const setupGlide = $glide => {
  ...
  for (const label in grid) {
    ...
    if (maxSize) {
      ...
      const peekRight = $glide.dataset['peekRight' + label]
   
      if (perView) {
        options.breakpoints[maxSize] = {
          perView: parseFloat(perView, 10)
        }
        if (peekLeft && peekRight) {
          
          options.breakpoints[maxSize].peek = {
            before: parseInt(peekLeft, 10),
            after: parseInt(peekRight, 10)
          }
        }
        options.breakpoints[maxSize].type = "carousel"
